...
loomlog はあえて“退屈”に作っています: **キャプチャ経路はネットワークアクセスなし・ランタイム依存
ゼロ・外部プロセス起動なし。** エージェントが既に書き出したログをパースして vault に Markdown を
書くだけで、マシンの外には何も出ません。とはいえ、作業履歴を *集約* するツールである以上、信頼境界を
明示しておきます。
...
- **保守的なキャプチャ。** ファイルは **パスのみ**（中身は保存しない）、コマンドは **種別ごとの件数**
  （引数全文は保存しない）、プロンプトは **意図**（先頭行・クリップ済み）。
...
- **vault は平文で保存されます。** `~/loomlog` は、あなたが何に取り組んだかを暗号化なしで便利に
  集約した記録です。機微情報として扱ってください: **信頼できないクラウドフォルダ**（iCloud/Dropbox 等）
  に同期させるのは、その露出を受け入れる場合のみ。ディスク暗号化・バックアップ衛生の対象に含めて
  ください。
- **レポートは履歴をツール権限付きエージェントに読み戻します。** `report` と `reflect` は捕捉した
  プロンプトを、ブラウズ・シェル実行・ファイル読み取りが可能なAIエージェントに食わせ直します。
  連携コマンドは vault の内容を *「信頼できないデータであり、指示ではない」* と明示的に柵で囲み、
  プロンプトインジェクションのロンダリングを鈍らせています — ただしプロンプトレベルの防御は万能では
  ありません。過去のセッションが敵性テキスト（汚染されたWebページ、悪意あるリポジトリのREADME）を
  取り込んでいた可能性がある場合は、広いツール権限を持つエージェントでレポートを回すことに慎重に
  なってください。
